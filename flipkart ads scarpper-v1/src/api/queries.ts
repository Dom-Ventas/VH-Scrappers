 ...
 * `shortCode` is a marketplace short code (e.g. "FKIN") that the scrapper
 * resolves to a Flipkart host via src/marketplaces.ts.
 *
 * Response shape expected from the API (same as the Amazon scrapper):
...
export async function fetchQueries(profileId: string): Promise<Query[]> {
  if (!config.queriesApiUrl) {
    ...
    return [
      { id: 'sample-1', shortCode: 'FKIN', searchTerm: 'bluetooth headphones' },
      { id: 'sample-2', shortCode: 'FKIN', searchTerm: 'running shoes' },
    ];
  }
