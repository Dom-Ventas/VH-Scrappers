...
// Amazon renders the delivery promise as free text ("FREE delivery Thursday,
// 21 August", "Entrega GRATIS el jueves, 21 de agosto", "Lieferung Donnerstag,
// 21. August"). This module turns that text into a plain day count:
//
...
// the range wins — that is the promise Amazon is making).
...
/**
 * Amazon omits the year, so "3 January" scraped in December means next year.
...
function matchExplicitDate(
  ...
  // Whichever appears first in the text is the primary (standard) promise —
  // "FREE delivery <date> ... Or fastest delivery <earlier date>".
  const useDayFirst =
    dayFirst &&
